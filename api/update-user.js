...
//   // Handle preflight OPTIONS request with an early return
//   if (req.method === 'OPTIONS') {
//     // Adding a log to confirm this block is being hit
...
export default async function handler(req, res) {
  ...
  // Changed from `userId` to `id` to match the Vercel rewrite rule
  const { id } = req.query; 
  const { firstName, lastName, email } = req.body;

  // The check for the ID is now for `id`
  if (!id) {
    return res.status(400).json({ msg: 'User ID is required' });
  }
  ...
  try {
    ...
    // Use `id` for the findByIdAndUpdate function
    const updatedUser = await User.findByIdAndUpdate(
      id,
      { firstName, lastName, email },
      { new: true, runValidators: true }
