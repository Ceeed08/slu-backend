...
//   else if (req.method === 'PUT') {
//     try {
//       // Find the item and update it with the request body data
...
//       return res.status(200).json({ success: true, msg: 'Item updated successfully.', data: updatedItem });
//     } catch (err) {
//       console.error(err);
//       return res.status(500).json({ success: false, msg: 'Server error' });
//     }
...
// The function is now a standard, exportable module.
// It no longer handles CORS headers or the OPTIONS preflight request,
// as those are managed by the main `index.js` file.
export default async function(req, res) {
  // The database connection is handled by index.js.
  // The ID is now passed to this handler via `req.query` from the index.js router.
  const { id } = req.query;
